...
const rows = [
  { id: 1, qusetion: '전체 축구팀 목록을 팀이름 오름차순으로 출력하시오', answer: '우ㅜㅎ' },
  { id: 4.1, qusetion: '수원팀에서 골키퍼(GK)의 이름을 모두 출력하시오. (ID 모를 경우)' },
...
function Home() {
  ...
  return (<>
    <div>
      ...
      <table className="border:2px solid black">
        <thead>
          <tr>
            <th>No.</th>
            <th>문제</th>
        ...
        <tbody>
          {rows.map((i) => (
            <tr>
              <th>{i.id}</th>
              <td>{i.qusetion}</td>
              <td onClick={() => alert('답 : ' + i.answer)}>Cilk!</td>
            </tr>
          ))}
        </tbody>
      </table>
